...
import 'package:zeroclaw_flutter_ui/src/core/background_service.dart';
import 'package:zeroclaw_flutter_ui/src/core/app_theme.dart';
import 'package:zeroclaw_flutter_ui/src/core/startup_trace.dart';
import 'package:zeroclaw_flutter_ui/src/ui/dashboard_page.dart';
import 'package:zeroclaw_flutter_ui/src/ui/config_page.dart';
import 'package:zeroclaw_flutter_ui/src/ui/webview_page.dart';
import 'package:zeroclaw_flutter_ui/src/ui/log_page.dart';
import 'package:zeroclaw_flutter_ui/src/ui/chat_page.dart';
import 'package:tray_manager/tray_manager.dart';
import 'package:window_manager/window_manager.dart';
import 'package:windows_single_instance/windows_single_instance.dart';
import 'dart:io';
import 'package:zeroclaw_flutter_ui/src/ui/widgets/adaptive_action_bar.dart';
import 'package:zeroclaw_flutter_ui/src/ui/widgets/tv_focusable.dart';

void main(List<String> args) async {
  StartupTrace.mark('main() entered');
  WidgetsFlutterBinding.ensureInitialized();
  StartupTrace.mark('WidgetsFlutterBinding.ensureInitialized complete');
  StartupTrace.attachFirstFrameTrace();

  // Single Instance Check (Windows only)
  if (Platform.isWindows) {
    StartupTrace.mark('Windows single instance check start');
    await WindowsSingleInstance.ensureSingleInstance(
      args,
      "zeroclaw_flutter_ui_instance_key",
      onSecondWindow: (newArgs) {
        windowManager.show();
        windowManager.focus();
      },
    );
    StartupTrace.mark('Windows single instance check end');
  }

  // Initialize Window Manager (desktop platforms only)
  if (!Platform.isAndroid && !Platform.isIOS) {
    StartupTrace.mark('Window manager init start');
    ...
    StartupTrace.mark('Window manager init end');
  }

  if (Platform.isAndroid) {
    StartupTrace.mark('initializeBackgroundService start');
    await initializeBackgroundService();
    StartupTrace.mark('initializeBackgroundService end');
  }

  StartupTrace.mark('ServiceManager() create');
  final service = ServiceManager();
  StartupTrace.mark('ServiceManager.init start');
  await service.init();
  StartupTrace.mark('ServiceManager.init end');

  StartupTrace.mark('runApp start');
  runApp(ChangeNotifierProvider.value(value: service, child: const MainApp()));
  StartupTrace.mark('runApp end');
}
...
class MainApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    StartupTrace.markOnce('MainApp.build');
...
class _MainShellState extends State<MainShell>
  ...
  void initState() {
    super.initState();
    StartupTrace.markOnce('MainShell.initState');
  ...
  void didChangeDependencies() {
    ...
    StartupTrace.markOnce('MainShell.didChangeDependencies');
  ...
  @override
  Widget build(BuildContext context) {
    StartupTrace.markOnce('MainShell.build');
