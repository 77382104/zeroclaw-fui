...
import 'package:zeroclaw_flutter_ui/src/core/app_theme.dart';
import 'package:remixicon/remixicon.dart';

const String _githubRepoUrl = 'https://github.com/sipeed/zeroclaw_fui';
const String _zeroclawOfficialUrl = 'https://zeroclaw.io';
const String _sipeedOfficialUrl = 'https://sipeed.com';
const String _aboutProjectName = 'ZeroClaw';
...
class ConfigPageState extends State<ConfigPage> with WidgetsBindingObserver {
  ...
  final _rawConfigController = TextEditingController();
  ...
  void initState() {
    ...
    _rawConfigController.addListener(_markDirty);
  ...
  void _markDirty() {
    if (!_isDirty &&
        (_hostController.text != _originalHost ||
            ...
            _rawConfigController.text != _originalRawConfig)) {
  ...
  Future<void> _loadConfig() async {
    ...
    _rawConfigController.removeListener(_markDirty);

    if (mounted) {
      // Android 平台从 config.toml 读取配置
      if (Platform.isAndroid) {
        try {
          final rawConfig = await ZeroClawChannel.getConfig();
          final configPath = await ZeroClawChannel.getConfigPath();
          final config = await ZeroClawChannel.parseConfig();
          final host = Platform.isAndroid
              ? '0.0.0.0'
              : (config['host'] as String? ?? '127.0.0.1');
          final port = config['port'] as int? ?? 42618;
          final publicMode = Platform.isAndroid
              ? true
              : (config['public_mode'] as bool? ?? false);
          ...
          setState(() {
            ...
            _rawConfigController.text = rawConfig;
            ...
            _originalRawConfig = _rawConfigController.text;
            _isDirty = false;
          });
        } catch (_) {
          // 解析失败时使用默认值
          ...
          final configPath = await ZeroClawChannel.getConfigPath();
          setState(() {
            _hostController.text = Platform.isAndroid ? '0.0.0.0' : service.host;
  ...
  Future<void> _saveConfig() async {
    ...
    final isPublicMode = _hostController.text == '0.0.0.0';
    final rawConfigChanged =
        Platform.isAndroid && _rawConfigController.text != _originalRawConfig;
    ...
    try {
      if (port != null) {
        ...
        // Android 平台：写入 config.toml
        if (Platform.isAndroid) {
          final tomlContent = rawConfigChanged
          ...
          await ZeroClawChannel.saveConfig(tomlContent);
          _rawConfigController.text = tomlContent;
          await _loadConfig();
        }
      }
    } catch (e) {
...
''';
  ...
  Future<void> _reloadRawConfig() async {
    if (!Platform.isAndroid) {
      return;
    }
    final rawConfig = await ZeroClawChannel.getConfig();
    ...
    setState(() {
      _rawConfigController.text = rawConfig;
      _configPath = configPath;
      _originalRawConfig = rawConfig;
      _isDirty = false;
    });
    widget.onDirtyChanged?.call(false);
  }

  Future<void> _saveRawConfig() async {
    if (!Platform.isAndroid) {
      return;
    }
    final service = context.read<ServiceManager>();
    final wasRunning = service.status == ServiceStatus.running;
    await ZeroClawChannel.saveConfig(_rawConfigController.text);
    await _loadConfig();
    if (wasRunning) {
      await service.stop();
      await service.start();
          ...
          child: Column(
            ...
            children: [
              ...
              TextButton.icon(
                autofocus: true,
                onPressed: () =>
                    _launchExternalUrl(Uri.parse(_zeroclawOfficialUrl)),
                icon: const Icon(Icons.open_in_new),
                label: Text(l10n.zeroclawOfficial),
              ),
              TextButton.icon(
                onPressed: () =>
          ...
          child: Column(
            ...
            children: [
              ...
              FocusableTextField(
                ...
                prevFocusNode: Platform.isAndroid ? _aboutFocusNode : _publicModeFocusNode,
              ),
              const SizedBox(height: 16),
              ...
              if (Platform.isAndroid) ...[
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Display and edit config.toml here',
                ...
                Row(
                  children: [
                    OutlinedButton.icon(
                      onPressed: _reloadRawConfig,
                      icon: const Icon(Icons.refresh),
                      label: const Text('Reload'),
                    ),
                    const SizedBox(width: 12),
                    FilledButton.icon(
                      onPressed: _saveRawConfig,
                      icon: const Icon(Icons.save_outlined),
                      label: const Text('Save config.toml'),
                    ),
                  ],
                ),
              ],
